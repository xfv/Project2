...
def assemble(img_bgr, M):
    ...
    for i in range(len(img_bgr)):
        ...
        d_size_x = np.array([[0, cols-1], [0, cols-1]])
        d_size_y = np.array([[0, 0], [rows-1, rows-1]])
        if i == 0:
            y_begin = d_size_y[0,0] - y_min
            y_end = d_size_y[1,1] - y_min
            x_begin = d_size_x[0,0]
            x_end = d_size_x[1,1]
            print 'y_begin/end, x_begin/end:', y_begin, y_end, x_begin, x_end
            panorama[y_begin:y_end+1, x_begin:x_end+1, :] += img_bgr[i]
